...
void RelCacheTable::relCatEntryToRecord(RelCatEntry *x,union Attribute record[]){
  strcpy(record[0].sVal,x->relName);
  record[1].nVal=x->numAttrs;
  record[2].nVal=x->numRecs;
  record[RELCAT_FIRST_BLOCK_INDEX].nVal=x->firstBlk;
  record[RELCAT_LAST_BLOCK_INDEX].nVal=x->lastBlk;
  record[RELCAT_NO_SLOTS_PER_BLOCK_INDEX].nVal=x->numSlotsPerBlk;
}
